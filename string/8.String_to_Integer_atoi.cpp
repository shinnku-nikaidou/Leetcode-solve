...
// character or the end of the input is reached. The rest of the string is
...
class Solution {
private:
  std::optional<int> ascii2int(const char a) {
    if (a >= '0' and a <= '9')
      return std::optional<int>{a - '0'};
    return std::nullopt;
  }
...
public:
  int myAtoi(string s) {
    bool neg = false;
    std::vector<int> a{};
    enum class State { front, mid, end };
    auto state = State::front;

    for (int i = 0; i < s.size(); i++) {
      std::optional<int> m_num;
      switch (state) {
      case State::front:
        if (s[i] == '-') {
          neg = true;
      ...
      case State::mid:
        m_num = ascii2int(s[i]);
        if (m_num == std::nullopt) {
          state = State::end;
          goto out;
        } else {
          a.emplace_back(m_num.value());
        }
  ...
  out:;
    ...
    for (auto x = a.begin(); x != a.end(); x++) {
      if (ans != 0 and std::numeric_limits<int>::max() / ans < 10) {
        return neg ? std::numeric_limits<int>::min()
                   : std::numeric_limits<int>::max();
      };
      ans *= 10;
      if (ans != 0 and std::numeric_limits<int>::max() - ans < *x) {
        return neg ? std::numeric_limits<int>::min()
                   : std::numeric_limits<int>::max();
      };
      ans += *x;
